...
const lookupStar = async () => {
  const instance = await StarNotary.deployed();
  const id = document.getElementById("starLookupId").value;
  const starName = await instance.lookUpTokenIdToStarInfo(id);
  App.setStarNameResponse(`Star is ${starName}.`);
}
...
const App = {
  ...
  setStarNameResponse: function (message) {
    const status = document.getElementById('starNameResponse')
    status.innerHTML = message
  },
...
window.addEventListener('load', function () {
  ...
  if (typeof web3 !== 'undefined') {
    ...
    window.web3 = new Web3(new Web3.providers.HttpProvider('https://rinkeby.infura.io/v3/4dd8abd289884591a1b95c0bbee86282'))
  }
